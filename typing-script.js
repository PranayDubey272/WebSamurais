...
document.addEventListener('DOMContentLoaded', function () {
   ...
   function handleInput() {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        const isCurrentWord = i === typedWords.length; // Check if the word is the current word being typed

        let wordHTML;
        if (isTyped) {
            wordHTML = `<span class="correct-word${isCurrentWord ? ' current-word' : ''}">${word}</span>`;
        } else {
